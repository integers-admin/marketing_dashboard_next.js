...
const credential = process.env.GOOGLE_APPLICATION_CREDENTIALS_INTEGERS;

if (!credential) {
  throw new Error(
    "GOOGLE_APPLICATION_CREDENTIALS_INTEGERS is not set."
  );
}
...
export const fetchIntegersGA4Data = async () => {
  try {
    const properties = [
      { id: "527224147", name: "Int - Main_Marketing" },
      { id: "541423590", name: "integersmarket" },
      { id: "537726190", name: "reportinshort" },
    ];
  ...
  try {
    ...
    try {
      customer = client.Customer({
        customer_id: process.env.GOOGLE_ADS_CUSTOMER_ID_INTEGERS,
    ...
    try {
      ...
      if (queryError.message?.includes('invalid_grant')) {
        console.log("Token expired during current month query.");
    ...
    try {
      ...
      if (queryError.message?.includes('invalid_grant')) {
        console.log("Token expired during last month query!");
        return { success: false, data: null };
      }
      throw queryError;
    }
    ...
    console.log("Integers Google Ads Data fetched successfully");
    return responseData;
